...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
    ...
    <div class="bg-[var(--surface)] border border-[var(--hairline)] rounded-2xl p-5">
        ...
        <p class="font-display text-2xl font-semibold text-[var(--text)]">{{ $summary['pending_count'] }}</p>
    </div>
    ...
    <div class="bg-[var(--surface)] border border-[var(--hairline)] rounded-2xl p-5 flex flex-col justify-between">
        <div>
            ...
            <p class="font-display text-2xl font-semibold text-[var(--text)]">
    ...
    <form method="GET" class="flex items-end gap-2 flex-wrap">
        <div class="flex flex-col gap-1">
            ...
            <input
                ...
                class="bg-[var(--surface)] border border-[var(--hairline)] rounded-xl px-4 py-2 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--gold)] w-56"
            >
        </div>
        <div class="flex flex-col gap-1">
        ...
        @if (request()->anyFilled(['q', 'status', 'date_from', 'date_to']))
            <a href="{{ route('admin.transactions.index') }}" class="text-xs font-semibold px-4 py-2 rounded-xl text-[var(--text-muted)] hover:text-[var(--gold)]">
                Reset
            </a>
        @endif
    </form>
        ...
        <tbody>
            @forelse ($transactions as $trx)
                <tr class="border-b border-[var(--hairline)] last:border-0 hover:bg-[var(--surface-2)]/40">
                    ...
                    <td class="px-5 py-3">
                        <p class="font-semibold text-[var(--text)] truncate">{{ $trx->user->first_name ?? '(user dihapus)' }}</p>
...
<div id="withdrawModalBackdrop" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm px-4">
    <div class="bg-[var(--surface)] border border-[var(--hairline)] rounded-2xl w-full max-w-sm p-6 shadow-2xl">
        <h3 class="font-display text-lg font-semibold text-[var(--text)] mb-1">Tarik Saldo</h3>
        ...
        <form method="POST" action="{{ route('admin.transactions.withdraw') }}">
            ...
            <input
                ...
                class="w-full bg-[var(--surface-2)] border border-[var(--hairline)] rounded-xl px-3 py-2 text-sm text-[var(--text)] mb-6 focus:outline-none focus:border-[var(--gold)]"
            >
